...
const props = defineProps<{ categories: CategoryExpert[] }>()
...
type ImportRow = {
  ...
  category: string
  note: string
  status: 'valid' | 'email_not_found' | 'already_expert' | 'invalid_category'
  statusLabel: string
  userId?: string
}
...
async function downloadTemplate() {
  ...
  const sampleData = [
    ['email', 'category', 'note'],
    ['user@example.com', props.categories[0]?.value ?? '', 'Contoh catatan opsional']
  ]
  const ws = XLSX.utils.aoa_to_sheet(sampleData)
  ws['!cols'] = [{ wch: 32 }, { wch: 24 }, { wch: 44 }]
  const wb = XLSX.utils.book_new()
  XLSX.utils.book_append_sheet(wb, ws, 'Experts')
  const guide = [
    ['value', 'nama kategori'],
    ...props.categories.map(c => [c.value, c.name])
  ]
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(guide), 'Kategori Valid')
  XLSX.writeFile(wb, 'template-import-experts.xlsx')
}
...
async function handleFileChange(files: FileList | File[] | null) {
  ...
  try {
    ...
    // Get already-registered experts
    const userIds = [...profileMap.values()]
    const { data: existing } = userIds.length
      ? await supabase.from('experts').select('user_id').in('user_id', userIds).is('deleted_at', null)
      : { data: [] }
    const existingSet = new Set((existing ?? []).map(e => e.user_id))

    const validCategories = new Set([...props.categories.map(c => c.value), ''])
    ...
    parsedRows.value = rows.map((r, i) => {
      ...
      if (!email || !userId) {
        ...
        status = 'already_expert'; statusLabel = 'Sudah terdaftar'
      } else if (category && !validCategories.has(category)) {
        status = 'invalid_category'; statusLabel = 'Kategori tidak valid'
      }

      return { rowNum: i + 2, email: r['email'] ?? '', category, note, status, statusLabel, userId }
    })
  } catch (err: any) {
...
async function executeImport() {
  ...
  try {
    const { error } = await supabase.from('experts').insert(
      rows.map(r => ({ user_id: r.userId!, category: r.category || null, note: r.note || null }))
    )
    if (error) throw error
    toast.add({ title: `${rows.length} pakar berhasil diimport`, color: 'success', duration: 3000 })
    open.value = false
    emit('saved')
  } catch (err: any) {
...
const rules = computed(() => [
  ...
  },
  {
    ...
    desc: 'Nilai kolom category harus menggunakan value yang valid (bukan nama tampilan). Daftar value kategori tersedia di bawah dan pada sheet "Kategori Valid" di template.',
    slot: 'category'
  },
  {
    title: 'Batas data & duplikat',
    desc: 'Maksimal 500 baris per sekali import. User yang sudah terdaftar sebagai pakar akan dilewati (tidak error) dan ditampilkan di preview dengan status "Sudah terdaftar".',
    slot: null
  }
])
</script>
  ...
  <UModal v-model:open="open" :ui="{ content: 'max-w-2xl' }">
    <template #header>
      <div class="flex items-center gap-3">
        ...
        <div>
          <p class="font-semibold text-highlighted">Import Data Pakar</p>
    ...
    <template #body>
      ...
      <div v-if="step === 1" class="space-y-3">
        <UAlert
          ...
          color="info"
          variant="soft"
          title="Baca panduan berikut sebelum mengimport data pakar"
        />
          ...
          <div v-for="(rule, i) in rules" :key="i" class="flex gap-3 p-3 rounded-lg bg-elevated">
            ...
            <div class="min-w-0">
              ...
              <div v-if="rule.slot === 'columns'" class="flex flex-wrap gap-1.5 mt-2">
                ...
                <UBadge color="neutral" variant="soft">category</UBadge>
                <UBadge color="neutral" variant="soft">note</UBadge>
              </div>
              <div v-if="rule.slot === 'category'" class="mt-2">
                <div class="flex flex-wrap gap-1.5">
                  <UBadge v-for="cat in categories" :key="cat.value" color="success" variant="soft">
                    {{ cat.value }}
                  </UBadge>
                  <span v-if="!categories.length" class="text-xs text-muted italic">Tidak ada kategori tersedia</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="flex items-center gap-3 pt-1 border-t border-default">
          ...
          />
          <p class="text-xs text-muted">Template berisi contoh data dan daftar kategori valid</p>
        </div>
      </div>
            ...
            <table class="w-full text-xs">
              <thead class="bg-elevated sticky top-0">
                <tr>
                  ...
                  <th class="px-3 py-2 text-left font-medium text-muted">Category</th>
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="px-3 py-2 text-highlighted max-w-40 truncate">{{ row.email }}</td>
                  <td class="px-3 py-2 text-muted">{{ row.category || '-' }}</td>
                  <td class="px-3 py-2 text-muted max-w-30 truncate">{{ row.note || '-' }}</td>
                  <td class="px-3 py-2">
